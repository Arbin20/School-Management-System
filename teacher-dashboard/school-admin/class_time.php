						...
						<tbody>
							<?php
								...
								while ($row = mysqli_fetch_assoc($result)) {
									...
									echo "<td><a href='class_time.php?edit_class_time=".$row['id']."' class='btn btn-warning'>Ndrysho</a></td>";
									echo "<td><button onclick='confirm_delete(".$row['id'].")' class='btn btn-danger delete-hidden'>Delete</button><span class='text-danger shwo-text'>Ndryshoni në vend që të fshini.</span></td>";
									echo "</tr>";
								}
							?>
						</tbody>
				...
				<div class="col-md-6">
					...
					<form action="" method="post">
						<div class="form-group">
							<label for="">Ndryshoni orarin e klasave</label>
							<input type="text" name="class_time" class="form-control" value="<?php echo get_class_time_by_id($class_time_id); ?>">
						</div>
						<div class="form-group">
							<input type="submit" name="update_class_time_submit" value="Ndryshoni orarin" class="btn btn-info">
						</div>
					</form>
					<?php } else { ?>
					<?php add_class_time(); ?>
					<form action="" method="post">
						<div class="form-group">
							<label for="">Orari i Klasës</label>
							<input type="text" name="class_time" class="form-control">
						</div>
						<div class="form-group">
							<input type="submit" name="class_time_submit" value="Shto Orarin" class="btn btn-info">
						</div>
					</form>
					<?php } ?>
				</div>
